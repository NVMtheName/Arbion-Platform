...
WITH active_ai AS (
  SELECT i.id,i.user_id,i.financial_account_id,i.execution_mode,a.status AS account_status,p.status AS connection_status
  ...
  WHERE i.status='ACTIVE' AND i.execution_mode IN ('PAPER','SHADOW') AND m.automation_type='AI_AUTONOMOUS'
), active_shadow AS (
  SELECT * FROM active_ai WHERE execution_mode='SHADOW'
...
SELECT
	(SELECT count(*) FROM active_shadow),
	(SELECT count(*) FROM active_ai WHERE execution_mode='PAPER'),
	(SELECT count(*) FROM active_ai a JOIN nonlive_strategy_schedules s ON s.strategy_instance_id=a.id
		WHERE s.consecutive_failures>0 OR s.last_status='FAILED'),
	(SELECT count(*) FROM active_shadow a LEFT JOIN latest_reconciliation r ON r.user_id=a.user_id AND r.financial_account_id=a.financial_account_id
  ...
  (SELECT count(*) FROM nonlive_execution_records e JOIN automation_mandates m ON m.id=e.mandate_id WHERE m.automation_type='AI_AUTONOMOUS' AND e.mode<>'SHADOW'),
	(SELECT count(*) FROM strategy_instances i JOIN automation_mandates m ON m.id=i.automation_mandate_id WHERE m.automation_type='AI_AUTONOMOUS' AND i.execution_mode NOT IN ('PAPER','SHADOW')),
	(SELECT count(*) FROM nonlive_execution_records e JOIN automation_mandates m ON m.id=e.mandate_id WHERE m.automation_type='AI_AUTONOMOUS' AND e.mode NOT IN ('PAPER','SHADOW')),
