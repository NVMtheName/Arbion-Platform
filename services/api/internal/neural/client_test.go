...
func TestProposeShadowExcludesAccountIdentifiersAndNormalizesDecision(t *testing.T) {
	...
	transport := roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		if !ok || len(markets) != 1 || markets[0].(map[string]any)["symbol"] != "BTC" || markets[0].(map[string]any)["change_percent_1h"] != "1.25" || markets[0].(map[string]any)["history_status"] != "COMPLETE" || markets[0].(map[string]any)["liquidity_status"] != "AVAILABLE" || markets[0].(map[string]any)["spread_bps"] != "2.5" || markets[0].(map[string]any)["bid_depth_usd"] != "5000" || markets[0].(map[string]any)["liquidity_feed"] != "advanced_trade_public_book" {
			t.Fatalf("normalized markets missing: %#v", body)
		}
		recent, ok := body["recent_decisions"].([]any)
