...
func TestAnalyzeSendsBoundedFieldsAndNormalizesInsight(t *testing.T) {
	...
	transport := roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		if body["credential"] != secret || body["model"] != "gpt-5.6-luna" || body["prompt"] != "Explain diversification" || body["safety_identifier"] != strings.Repeat("a", 64) {
			t.Fatalf("unexpected request fields: %#v", body)
		}
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     make(http.Header),
			Body:       io.NopCloser(strings.NewReader(`{"insight":{"summary":"Diversification reduces concentration risk.","key_points":["Spread exposure."],"risk_flags":["Loss remains possible."],"limitations":["No live data."],"requires_current_data":false,"metadata":{"provider":"openai","model":"gpt-5.6-luna","input_usage":30,"output_usage":45,"request_id":"resp-safe","latency_ms":120}}}`)),
		}, nil
	})
	client := NewHTTPClient("http://ai.internal", "internal-token", &http.Client{Transport: transport})
	result, err := client.Analyze(context.Background(), "openai", "gpt-5.6-luna", []byte(secret), "Explain diversification", strings.Repeat("a", 64))
	if err != nil {
		t.Fatal(err)
	}
	if result.Summary == "" || result.Metadata.RequestID != "resp-safe" || result.Metadata.InputUsage == nil || *result.Metadata.InputUsage != 30 {
