...
- A forward-only credential-free deployment receipt and read-only release lineage reviewer that bind the saved host release to exact merged PR ancestry, bounded complete GitHub workflow populations, a contemporaneous pre-deploy backup, and rollback reference. Missing or inconsistent evidence fails closed; independent review, approval, enforcement, checkout/artifact attestation, and durable retention remain separate unresolved controls, never inferred from a successful deployment.
...
- A deterministic external-control review-draft generator that re-verifies saved GitHub/AWS evidence, rejects ambiguous or altered sources, maps 22 narrow assertions to catalog controls, and preserves exact file digests without copying raw evidence values. Explicit GitHub feature settings remain separate from missing license-related fields; current-main CodeQL analysis metadata and vulnerability-alert enablement are independently bound to the repository. Exact SNS account/region/topic binding separates operational alerts from security configuration; end-to-end delivery remains explicitly unavailable without a separately observed and reviewed receipt. Its output remains explicitly non-authoritative and cannot establish operating effectiveness, compliance, or certification.
