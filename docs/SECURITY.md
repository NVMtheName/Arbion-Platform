...
The connected Coinbase fill route is authenticated, owner-scoped, and `no-store`. It re-verifies the encrypted key is still View-only before every provider read, fixes the request to 50 spot fills, bounds provider bodies, validates exact positive price/size and nonnegative commission decimals, normalizes only BUY/SELL fill evidence, and rejects future or malformed timestamps. Raw provider identifiers, cursors, key material, and provider errors are discarded. A response requiring a proof token fails closed. The API and UI state that these are external Coinbase executions and explicitly expose no live-execution capability; the base provider registry continues to report orders unsupported.

The Coinbase order monitor applies the same authenticated owner scope, `no-store` policy, View-only re-verification, 50-row/body/cursor bounds, proof-token fail-closed behavior, exact-decimal validation, and future-time rejection. Only a strict allowlist of spot order states/types/time-in-force values and an identifier-free normalized projection reaches the UI. Free-form provider messages, order configuration/edit history, and every provider identity field are discarded. The endpoint returns `order_actions_available=false` and `live_execution_available=false`; no preview, create, edit, or cancel handler or provider method exists.

The Coinbase trading-cost route is authenticated, owner-scoped, and `no-store`, re-verifies View-only permissions, and fixes the provider request to a spot transaction summary. Required USD volume/fee values and maker/taker rates use exact-decimal validation; rates must remain between zero and one, fee/volume values must be nonnegative, and the provider tier label is length- and character-bounded. Balance, margin, tax, Coinbase Pro, volume-breakdown, and tier-range fields are never modeled. The response declares `order_preview_available=false`, `order_actions_available=false`, and `live_execution_available=false`; neither an order-specific fee estimate nor a preview endpoint exists in this boundary.
