...
Market-source status responses contain only declared source metadata, per-capability process-local attempt/success timestamps, consecutive-failure counts, bounded categories such as timeout or stale data, configured cache/pacing policy, and capped aggregate counts of valid cache lookups, cache hits, and provider attempts. The counters have no user, account, instrument, request, or provider-correlation dimension; reset on process restart; and are explicitly not a provider quota or remaining-credit balance. They never expose raw provider errors, response bodies, request URLs, headers, correlation values not already approved as observation provenance, or credentials. Invalid caller input never enters request telemetry, a cache hit does not create a provider-success event, and status metadata is never presented as an executable quote or provider event timestamp.
