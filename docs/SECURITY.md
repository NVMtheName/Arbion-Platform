...
Releasing the conservative account claim requires an authenticated, CSRF-protected owner command with the expected state version and an explicit non-live confirmation. The database locks and terminally completes only an active or paused owner-scoped instance, advances its optimistic version, and appends immutable transition evidence. The service separately records a safe audit event. It cannot delete history, modify the PAPER ledger, contact a provider, or send a broker order.
