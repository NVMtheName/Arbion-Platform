...
The Coinbase order-preview route is authenticated, owner-scoped, CSRF-protected, and `no-store`. It accepts only a canonical base asset, BUY/SELL, and a positive non-exponential exact decimal with bounded precision. The adapter fixes the provider request to a USD spot market IOC preview, uses BUY quote size or SELL base size, sends no client order ID or portfolio override, and re-verifies View/no-Transfer permission first. Provider totals, commission, book prices, estimated fill, warnings, and errors are schema/decimal bounded; raw errors collapse into a small safe category set. The Coinbase preview ID is validated but discarded and never reaches the browser, PostgreSQL, audit metadata, or Neural Engine. Responses always assert that no order was created, submission is unavailable, and AI has no execution authority. The provider interface contains no create, replace, cancel, convert, deposit, withdrawal, or transfer operation.

The Coinbase order-intent create/list/review routes are authenticated, owner-scoped, `no-store`, and CSRF-protected for mutations. Creation accepts a browser-generated bounded idempotency key and persists only server-normalized preview evidence after validating exact requested currency, safe message categories, positive ready-state values, provider Trade capability, and a short evidence age. Reusing the key with a changed payload fails closed. Immutable database hashes bind request and evidence; provider IDs and credentials are absent. Proposal review requires exact optimistic version, unexpired ready evidence, a still-active same-owner Coinbase account with the recorded Trade capability, and a newly consumed TOTP step. Recovery codes are rejected, replay is prevented by the durable factor counter, and the raw code is cleared after use. The resulting `USER_APPROVED_NONEXECUTABLE` record has scope `PROPOSAL_REVIEW_ONLY`; it explicitly leaves risk approval, AI execution authority, submission, and live execution false. No provider call follows review.
