...
The Coinbase order-intent create/list/review routes are authenticated, owner-scoped, `no-store`, and CSRF-protected for mutations. Creation accepts a browser-generated bounded idempotency key and persists only server-normalized preview evidence after validating exact requested currency, safe message categories, positive ready-state values, provider Trade capability, a short evidence age, and the same-time exact product-rule snapshot. Reusing the key with a changed payload fails closed. Immutable database hashes bind request, preview, and product controls; provider IDs and credentials are absent. Proposal review requires exact optimistic version, unexpired ready evidence, an enabled stored market-IoC product snapshot, a still-active same-owner Coinbase account with the recorded Trade capability, and a newly consumed TOTP step. Recovery codes are rejected, replay is prevented by the durable factor counter, and the raw code is cleared after use. The resulting `USER_APPROVED_NONEXECUTABLE` record has scope `PROPOSAL_REVIEW_ONLY`; it explicitly leaves risk approval, AI execution authority, submission, and live execution false. No provider call follows review.
