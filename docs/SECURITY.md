...
Every strategy instance now stores the capital bucket copied from its exact immutable mandate version and enforces that binding with a same-owner database foreign key. A partial unique index rejects a second active or paused instance for that bucket even under concurrent initialization. PAPER starting cash is parsed as an exact bounded decimal and cannot exceed the fixed allocation or absolute percentage cap after protected capital. This closes direct bucket reuse while leaving aggregate reservation accounting across separate buckets explicitly unresolved.
