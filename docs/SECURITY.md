...
The implemented Arbion Insight flow is intentionally data-isolated. An authenticated entitled user can submit at most 2,000 bytes of text using an active saved provider/model preference. The request includes no financial account, portfolio, broker, quote, news, or live-market context and exposes no tools. OpenAI requests set `store: false`, use low reasoning effort, cap output, and require a strict JSON schema. A Redis counter allows 12 requests per user per hour and fails closed if unavailable. Audit events retain provider/model identifiers, byte and token counts, outcome, latency, and provider request ID only; prompt text, model output, credentials, and private chain-of-thought are excluded.
