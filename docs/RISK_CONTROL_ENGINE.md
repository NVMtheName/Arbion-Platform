...
The package imports no connector or Neural Engine code and exposes no broker-write operation. The authenticated evaluation service builds normalized snapshots, loads applicable durable breakers, invokes the gate, and atomically persists its evidence with the non-live result. Both manual and guarded scheduled evaluation call this same service. An authenticated founder can engage and release durable automation- and account-scoped emergency stops through owner-facing APIs and UI. Both require an explicit confirmation and operator reason, are owner-scoped and audited, and never request a broker action. An account stop denies new actions across every Arbion automation and non-executing proposal tied to that exact financial account while leaving other accounts independent; monitoring and immutable evidence may continue. Global and user breaker mutation workflows, administrative dual control, realized daily P/L ingestion, and broader orchestration remain deferred.
