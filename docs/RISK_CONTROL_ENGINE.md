...
The broader policy language, production numerical defaults, jurisdictional rules, realized-loss calculation, cross-account aggregation, live execution reservation accounting, additional breaker recovery policies, administrative dual control, audit integrity, and formal live-trading safety case remain deferred. Manual Coinbase proposals account for immutable one-minute cash/asset reservations and fail closed on a concurrent snapshot change. Non-live strategy instances use durable lifecycle-bound aggregate policy reservations. The implemented deterministic gate authorizes only current manual or guarded scheduled PAPER/SHADOW evaluation; it has no broker trading capability.
...
The package imports no connector or Neural Engine code and exposes no broker-write operation. The authenticated evaluation service builds normalized snapshots, loads applicable durable breakers, invokes the gate, and atomically persists its evidence with the non-live result. Both manual and guarded scheduled evaluation call this same service. An authenticated founder can engage and release durable automation-, account-, and owner-wide user-scoped emergency stops through owner-facing APIs and UI. A current active superadmin can additionally engage one durable Arbion-global stop; release requires a fresh TOTP code that is consumed once. Every stop requires an explicit confirmation and operator reason, is audited, and never requests a broker action. An account stop denies new actions tied to that exact financial account, the owner-wide stop covers the authenticated owner's resources, and the global stop applies to every risk-gated action. Monitoring and immutable evidence may continue while a stop is active. Administrative dual control, realized daily P/L ingestion, and broader orchestration remain deferred.
