...
For external failure delivery, create a dedicated SNS topic and a confirmed operator subscription. Grant the host backup identity only `sns:Publish` to that exact topic, put `ARBION_ALERT_TOPIC_ARN=<topic-arn>` in root-owned `/etc/arbion/ops-alert.env`, and install `arbion-ops-alert@.service`. Backup, freshness-monitor, and Docker-cache-maintenance failures invoke the publisher with only the unit name, host name, and UTC timestamp; alerts contain no database contents or credentials.
