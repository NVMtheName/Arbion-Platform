...
Strategy parameters and schedule conditions are validated and normalized server-side. Saving either creates a new immutable mandate version and returns the mandate to DRAFT; the user must review and mark that version READY before initializing or evaluating its exact matching strategy instance. Initialization persists the immutable version's same-owner capital bucket and an exact durable capital reservation. PAPER starting cash must fit within its protected absolute capacity; SHADOW requires a fixed dollar capacity or an explicit percentage-bucket cap. Direct bucket reuse is rejected. Multiple active or paused instances may share a financial account only when their exact same-currency reservation amounts share one explicit fixed-allocation account ceiling and remain within it; otherwise the existing account claim blocks initialization. Paused strategies retain their reservation, and completion releases it atomically without deleting its history. This is simulation policy accounting only: it neither locks broker funds nor grants broker authority. Manual requests supply an event ID; scheduled runs derive one from the durable due time. The database atomically claims that identity with risk evidence, non-live execution evidence, the Decision Journal entry, and any PAPER-only state/accounting mutation.

Evaluation failures remain fail-closed but return stable, owner-safe diagnostic codes for an inactive strategy, changed bound configuration, invalid saved parameters, unavailable PAPER state, stale provider timestamps, and an option chain with no contract matching the saved filters. Manual and scheduled controls pair those codes with bounded recovery guidance while keeping raw provider errors private. These failures create no execution record and never relax the risk engine or broker-write boundary.
