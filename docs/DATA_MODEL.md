...
Connected Coinbase order preview is an ephemeral, non-executing owner-scoped projection. Arbion accepts only a canonical USD spot product, BUY or SELL side, and one exact positive size; a buy size is USD quote currency and a sell size is base asset. The normalized result contains bounded estimated totals, commission, bid/ask, fill price, warnings, block categories, retrieval time, and whether the stored provider key has Trade permission. The provider preview identifier is deliberately discarded, the preview is never persisted or joined to AI output, mandates, orders, fills, positions, tax lots, or journals, and no current model can address or create a provider order. Durable intent, approval, attempt, provider-correlation, execution-event, and reconciliation records remain prerequisites for a separately approved live-execution boundary.
