...
The permanent architecture requires normalized durable concepts for Automation Mandates and immutable versions, capital buckets and reservations, strategy definitions/instances/state transitions, structured Arbion Intents, provider-independent Order Intents and execution events, approvals, account capabilities, reconciliation observations, circuit breakers, and the Decision Journal. Mandates, buckets, non-live instances/transitions, circuit breakers, risk evaluations, non-live execution evidence, Decision Journal entries, and non-executing Coinbase proposal/review evidence are implemented; live orders, execution approvals, reconciliation, and reservations remain conceptual.
...
Coinbase reuses the same provider-neutral tables without a schema migration. A financial `provider_connections` row uses provider `coinbase`; its structured Coinbase App key name, ECDSA private key, verified portfolio ID, and verified provider Trade-permission flag exist only inside the Vault ciphertext. View is required, Trade is optional, and Transfer is rejected. API connection models expose none of those credential values. The durable `financial_accounts` row represents the permissioned Coinbase portfolio with a server-only `portfolio:{provider UUID}` identity, a masked label, USD display currency, explicit crypto-holdings and non-executing order-preview capabilities, and unsupported provider-write/transfer capabilities. Individual wallet balances and holdings are fetched on demand and are not persisted.

Coinbase portfolio valuation is also an on-demand read model, not a durable ledger. Current quantities come from the authenticated account adapter using the key's View permission; last trade, bid, ask, venue, and timestamps come from the independent keyless Exchange market adapter. The API derives bounded exact-decimal observed values in memory and returns per-position pricing status and aggregate coverage. It does not persist price snapshots, observed totals, inferred pegs, cost basis, or unavailable-asset estimates.
...
Connected Coinbase trading costs are another ephemeral owner-scoped projection. The normalized snapshot contains only spot scope, a bounded provider tier label, exact maker/taker rates, exact USD Advanced Trade volume and fees, exact provider total fees, the provider pricing-model flag, and retrieval time. It is never persisted or joined to orders, strategies, positions, fills, tax lots, or journal records. Arbion does not infer the provider's reporting window, calculate next-tier progress, or treat the tier summary as an order-specific quote.

Connected Coinbase order preview begins as an ephemeral, non-executing owner-scoped projection. Arbion accepts only a canonical USD spot product, BUY or SELL side, and one exact positive size; a buy size is USD quote currency and a sell size is base asset. The normalized result contains bounded estimated totals, commission, bid/ask, fill price, warnings, block categories, retrieval time, whether the stored provider key has Trade permission, and a same-time authenticated Coinbase product-rules snapshot. That snapshot preserves exact base/quote increments and minimum/maximum sizes, bounded product status, and normalized market-IoC blocking reasons. The provider preview identifier is deliberately discarded. The owner can persist only the normalized evidence as a non-executing proposal; it is never joined to mandates, fills, positions, tax lots, or provider-order identity, and no current model can address or create a provider order.

Migration `00019_nonexecuting_order_intents.sql` adds `order_intents`, `order_intent_previews`, `order_intent_reviews`, and `order_intent_events`. An intent belongs to one owner and same-owner financial account, records a UI or internal AI source, exact normalized market-IoC terms, a SHA-256 request hash, and a per-owner idempotency key. Preview evidence is immutable, expires after one minute, contains no Coinbase preview ID, and is bound by a SHA-256 evidence hash. Review evidence is append-only, TOTP-only, and fixed to `PROPOSAL_REVIEW_ONLY`; the only mutable transition is version 1 `REVIEW_REQUIRED` to version 2 `USER_APPROVED_NONEXECUTABLE`. Database triggers reject mutation/deletion of evidence and any other intent transition. The schema has no provider order ID, client order ID, submission state, attempt, reconciliation, fill, position, mandate, capital, or execution-approval field. `BLOCKED` proposals cannot be reviewed.

Migration `00020_order_intent_product_rules.sql` extends immutable preview evidence with an all-or-none Coinbase spot-product snapshot. Positive exact increments and bounds, a bounded status, a safe normalized reason array, the derived market-IoC availability flag, and an observation time equal to the provider preview time are database-constrained. Legacy version-19 proposals may contain no product snapshot and consequently cannot pass current review validation. The migration adds no provider identifier or execution state.
...
`market_watchlist_items` is a durable owner preference with a generated identifier, owning user, fixed `CRYPTO` asset class, canonical 1-12 character symbol, fixed `USD` quote currency, and creation time. It contains no account, connection, credential, provider identifier, quantity, price, cost basis, signal, strategy, order, transfer, or execution field. The owner/symbol uniqueness constraint and owner-ordered index support a hard 12-item service boundary. Current Coinbase observations are joined into the API response in memory and are never written to this table; an unavailable observation leaves the preference intact without an estimated value.
