...
Connected Coinbase order preview begins as an ephemeral, non-executing owner-scoped projection. Arbion accepts only a canonical USD spot product, BUY or SELL side, and one exact positive size; a buy size is USD quote currency and a sell size is base asset. The normalized result contains bounded estimated totals, commission, bid/ask, fill price, warnings, block categories, retrieval time, and whether the stored provider key has Trade permission. The provider preview identifier is deliberately discarded. The owner can persist only the normalized evidence as a non-executing proposal; it is never joined to mandates, fills, positions, tax lots, or provider-order identity, and no current model can address or create a provider order.
