...
## Unified holdings ledger semantics

`/accounts` is the provider-neutral portfolio ledger. It fetches each active account independently so one provider or authorization outage cannot erase, replace, or block the other account rows. The UI exposes partial coverage by account and by metric. It does not interpret a missing refresh as a zero balance, a closed position, or permission to alter an active strategy.

For Schwab positions, `averagePrice`, `currentDayProfitLoss`, `currentDayProfitLossPercentage`, and the applicable long/short open-profit-loss value remain provider-reported account facts. Current unit price is an exact display derivation from the provider's absolute position market value divided by absolute quantity and the standard instrument multiplier (100 for options, 1 otherwise). Open return percentage is an exact display derivation against provider average price, absolute quantity, and the same multiplier. Both derivations are labeled `PROVIDER_POSITION_MARKET_VALUE_PER_UNIT`; they do not claim an independent quote or executable price.

For Coinbase spot positions, current price and position value use the existing approved last-trade observation, while rolling 24-hour amount and percentage compare that observation with the exact public product-statistics open. The stable USDC/USD redemption reference receives an explicit zero movement rather than a fabricated venue tick. Coinbase spot inventory does not provide authoritative tax-lot purchase price through the implemented account reads. Average purchase price and total return therefore remain visibly unavailable; Arbion never reconstructs them from the bounded first page of fills.

Summary totals state metric coverage. Holdings value may include every priced row, day/24-hour movement includes only rows with the applicable provider observation, and total return includes only rows with provider cost/performance facts. Values in different currencies must not be silently combined.
...
- Implemented: owner-scoped Coinbase App wallet/vault inventory reconciled with Coinbase Portfolio Breakdown availability, retaining exact total, available-to-trade, and staked-or-otherwise-unavailable quantities without double counting; up to 32 holdings are priced against approved USD tickers with per-position venue/time evidence, while USDC uses a separately labeled Coinbase 1:1 USD redemption reference and all other coverage gaps remain unestimated.
...
- Implemented: AI Shadow Engine 1-hour and 6-hour trend facts derived with exact decimal math only from contiguous trailing Coinbase fifteen-minute candles; immutable evidence carries coverage, feed quality, and timestamps, while partial, stale, and unavailable history remains explicit.
...
- Implemented: a unified Coinbase and Schwab holdings ledger with provider/account filters, symbol/account search, exact quantities, average purchase price where authoritative, current price, day/24-hour movement, market value, and coverage-labeled total return; Coinbase cost basis remains explicitly unavailable rather than inferred.
