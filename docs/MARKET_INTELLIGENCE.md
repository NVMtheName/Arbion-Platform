...
- Implemented: an owner-scoped, CSRF-protected Coinbase spot market-IoC preview that accepts one exact USD buy size or base-asset sell size, returns normalized provider cost/fill evidence, discards the provider preview identifier, and cannot create an order.
