...
The current implementation remains a modular monolith plus one dedicated AI service. Email/password authentication, AI-provider connection verification/configuration, bounded read-only Arbion Insight analysis, a bounded OpenAI-to-Coinbase proposal-only bridge, private Schwab/Coinbase account connectivity, real non-executing Coinbase order previews, and durable capital-policy-bound Coinbase proposals with deterministic risk evidence and TOTP-backed owner review are implemented. Live or automated trading, execution approval for those proposals, order submission/cancellation, tool-using AI, and legacy Flask migration remain out of scope until explicitly designed and approved.
