...
The Connection Hub also projects an owner-scoped continuity map from current control-plane records, never from provider calls. Financial connections report exact READY/PAUSED mandate and ACTIVE/PAUSED strategy-instance counts through their owned accounts. AI connections separately report current protected mandates, runtimes bound through each instance's pinned immutable version, retained automation identity, and default-model selection. The UI blocks a destructive control when the corresponding current projection is protected, while verified AI-key rotation, account refresh, and Schwab renewal remain available because they preserve connection identity. This browser guidance is advisory: every disable, removal, or disconnect command independently rechecks the same owner-scoped database dependencies, so a stale page cannot bypass continuity protection. The projection contains no credentials, account identifiers, holdings, symbols, mandate objectives, model prompts, or provider payloads and adds no broker or live-execution capability.

Connection lifecycle and automation attachment share one deterministic PostgreSQL advisory-lock protocol. A destructive connection command holds its connection lock from the authoritative dependency recheck through status or credential retirement. Mandate creation, mandate replacement, READY/PAUSED transitions, and strategy initialization acquire the same connection locks in sorted order, then re-read active owner/category/account state after locking before they write. Strategy initialization also row-locks and rechecks the current READY mandate version. Therefore a command that validated an active connection earlier cannot attach after a concurrent disable, disconnect, or removal, and a lifecycle command that waits for an attachment sees every committed dependency covered by its protection policy before it can proceed. A conflict fails closed and leaves existing strategy evidence unchanged.

## Implemented Coinbase account connection

The founder-phase Coinbase connector uses a user-created Coinbase App secret API key because Coinbase currently reserves third-party OAuth client creation for approved partners and directs single-owner server integrations to API keys. Arbion's primary form accepts the full CDP key name plus an ECDSA P-256 private key; downloaded key JSON is optional. The browser rejects a plain one-line secret with explicit guidance because it is an Ed25519 or expired legacy credential, not the ECDSA private key Coinbase App accepts. Quoted values, literal multi-line PEM, and Coinbase's documented one-line `\n` representation are normalized into one canonical form before verification and encrypted storage. Go generates a unique ES256 request-bound JWT with a two-minute lifetime for every provider call and sends it only to `https://api.coinbase.com`. The private key is never returned, logged, committed, or sent to Python.

Connection verification calls Coinbase's key-permission endpoint and requires `can_view=true` and `can_transfer=false`. `can_trade` may be present and is retained only as a provider capability fact; it does not grant Arbion risk approval, submission, live execution, or AI authority. A key with asset-transfer authority is rejected. Operators should additionally restrict the key to the intended Coinbase portfolio and the production host IP. Coinbase requires the ECDSA signature option for Coinbase App APIs; Ed25519 keys are rejected by the adapter.

Enrollment failures are separated without exposing provider bodies or credential material: malformed/non-ECDSA key formats, Coinbase authentication rejection, and unsafe permission combinations have distinct public error codes and guidance. The audit event records only the provider and safe error code.

The connector paginates the authenticated Advanced Trade account inventory with bounded pages and normalizes it as one `Coinbase Portfolio` financial account. USD available/held balances come from that account inventory. Crypto holdings combine two permissioned read surfaces: Coinbase App `GET /v2/accounts` supplies the bounded wallet/vault inventory, while Advanced Trade Portfolio Breakdown supplies explicitly available-to-trade quantities. Arbion aggregates exact Coinbase App totals by currency, reconciles the overlapping Advanced Trade total without summing the same balance twice, and conservatively treats quantity absent from Advanced Trade availability as unavailable to trade. The UI labels that difference `Staked / unavailable` because it can include rewards balances, staking, vaults, open orders, restrictions, or another provider coverage difference. Reconciliation preserves changes in that unavailable quantity as exact evidence; it treats them as non-blocking only when available-to-trade is identical and both snapshots' component sums exactly equal their totals. It never labels the cause as staking or rewards without provider proof. Provider account UUIDs and the portfolio ID remain server-only, and Arbion does not invent a cross-asset portfolio value or cost basis. Sync is duplicate-safe through the existing `(provider_connection_id, provider_account_id)` inventory key. Disable, enable, sync, and disconnect share the existing owner-scoped lifecycle; disconnect deletes Arbion's encrypted key but does not claim to delete the key in Coinbase, so the owner should revoke it in the Coinbase portal when appropriate.

The authenticated `GET /api/accounts/{id}/portfolio/crypto` read model combines those current holdings with a separately keyless Coinbase Exchange `GET /products/{product_id}/ticker` observation for at most 32 unique USD products per refresh. Exact-decimal multiplication produces an informational observed value based on the ticker's last trade; bid, ask, venue, feed quality, and provider time remain attached to every venue-priced position. USDC in a USD-base account is handled as one explicit exception: its exact inventory is valued at [Coinbase's documented 1:1 USD redemption reference](https://help.coinbase.com/en/coinbase/getting-started/crypto-education/usdc) and labeled separately from market observations, with no fabricated bid, ask, venue, provider timestamp, cost basis, or performance claim. The reference-valued USDC position participates in portfolio value and allocation but never becomes a venue chart selector. A missing USD product for any other asset or a market outage leaves the holding visible and unpriced; Arbion does not infer any other stablecoin peg, cross rate, cached database estimate, or cost basis. The response is owner-scoped, `no-store`, and always declares that live execution is unavailable.
...
The non-executing `financial.OrderPreviewProvider` first performs an authenticated `GET /api/v3/brokerage/products/{product_id}` read for the exact USD spot product, then requests the provider preview. Arbion preserves exact base/quote increments and minimum/maximum sizes, product status, and the provider's disabled, trading-disabled, cancel-only, limit-only, post-only, and auction flags as bounded normalized evidence. The requested BUY quote size or SELL base size is checked with exact rational arithmetic against the applicable increment and bounds. Unsafe metadata fails closed; a valid restriction produces a durable blocked proposal. The product observation and preview share one timestamp and expire together after one minute.
