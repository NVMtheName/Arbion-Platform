...
The owner-scoped SHADOW scorecard groups those immutable marks by 1-hour and 24-hour horizon. It reports exact sample, favorable, unfavorable, and flat counts plus observed favorable rate and average directional movement. Horizons never mix, missing marks are never estimated, and the UI keeps the interpretation at `INSUFFICIENT_SAMPLE` until at least 20 marks exist for that horizon. Even after that floor, `OBSERVATIONAL` is a descriptive label only—not prediction accuracy, statistical significance, realized performance, or authority to trade.
