...
Secure provider connection management, credential verification, model discovery, default provider/model preferences, a bounded read-only Arbion Insight flow, a proposal-only Coinbase research flow, and a provider-independent Coinbase/Schwab AI Shadow Engine are implemented. General chat, automatic intent routing, streaming, model tools, execution approval, broker submission, and live execution remain unimplemented.
