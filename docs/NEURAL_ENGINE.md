...
The model may return `ABSTAIN`, or one `PROPOSE` decision containing an allowlisted symbol, BUY/SELL, and a positive notional no greater than the mandate ceiling. It is instructed to use the bounded recent-decision context to avoid repetitive reasoning, but that context is advisory evidence rather than authority. Python validates the schema and envelope; Go revalidates price, quantity, mandate/model metadata, and current SELL availability. Abstention appends an immutable `AI / ABSTAIN` Decision Journal entry with no risk or execution record. A proposal enters the same provider-independent deterministic risk engine used by other financial actions. That gate also enforces a platform-owned one-hour cooldown on an identical autonomous SHADOW symbol and side, using only prior immutable `WOULD_HAVE_SUBMITTED` records from the same strategy instance; missing or invalid recent-action evidence fails closed. An allowed result reaches only the Shadow adapter and is persisted as `WOULD_HAVE_SUBMITTED`; a cooldown or other denial is persisted as `RISK_DENIED`. Neither result changes broker holdings, creates a preview, reserves live capital, or calls an order endpoint. Stable failures distinguish Arbion's own hourly decision budget (`AI_DECISION_BUDGET_EXHAUSTED`) from an upstream model-provider throttle (`AI_PROVIDER_RATE_LIMITED`); both fail before any financial action and provider error text is never returned.
