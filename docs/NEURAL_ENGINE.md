...
Secure provider connection management, credential verification, model discovery, default provider/model preferences, a bounded read-only Arbion Insight flow, a proposal-only Coinbase research flow, and an OpenAI-or-Claude Coinbase/Schwab AI Shadow Engine are implemented. General chat, automatic intent routing, Gemini Shadow decisions, streaming, model tools, execution approval, broker submission, and live execution remain unimplemented.
