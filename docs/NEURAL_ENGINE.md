...
Secure provider connection management, credential verification, model discovery, default provider/model preferences, a bounded read-only Arbion Insight flow, a proposal-only Coinbase research flow, and an OpenAI/Claude/Gemini Coinbase/Schwab AI Shadow Engine are implemented. General chat, automatic intent routing, streaming, model tools, execution approval, broker submission, and live execution remain unimplemented.
...
## Implemented autonomous AI Shadow Engine

An `AI_AUTONOMOUS` mandate can initialize only as `SHADOW` with `FULL_AUTONOMOUS`, an active owned OpenAI, Anthropic, or Gemini connection, one approved fixed model route, an isolated capital bucket, a 1–8 symbol allowlist, a bounded objective, and an exact positive per-decision notional ceiling. OpenAI routes are Luna/Terra/Sol; Anthropic routes are pinned Haiku 4.5/Sonnet 5/Opus 5 releases; Gemini routes are stable Flash 3.5/Flash 3.6/Flash 3.7 releases for Fast/Core/Deep respectively. Preview and latest aliases are excluded. Provider and exact model are frozen together, and a mismatch fails before credential retrieval. Initialization creates an `ai_shadow` instance in `AI_MONITORING`; it creates no PAPER cash and exposes no LIVE option. Coinbase mandates may use a `CONTINUOUS` cadence, while Schwab mandates use the guarded U.S. regular-session calendar. Each financial account keeps a separate immutable mandate, instance, circuit breaker, schedule, and journal.

The mandate review UI can revise the bounded objective and per-decision ceiling through an AI-specific validated endpoint. Every revision creates a new immutable `DRAFT`; it never evaluates the model or changes broker permissions. The reviewed version must be marked `READY`, and any older active or paused non-live instance must be finished before the replacement `SHADOW` instance can be initialized. Historical decisions remain attached to their original mandate version.

Go refreshes current balances and only allowlisted position facts. Schwab market facts come from its authenticated read-only quote route; Coinbase market facts come from the keyless public Exchange observation service. Coinbase decisions also receive deterministic 1-hour and 6-hour price changes only when the exact trailing fifteen-minute candles are contiguous. The request discloses the observed/expected interval count, history status, feed, quality, and timestamp; missing buckets are never filled, and stale or incomplete windows remain explicitly unavailable or partial. Schwab history remains explicitly unavailable until an approved normalized historical feed exists. Each request also receives at most six validated, same-instance decisions from the prior six hours, containing only decision, symbol, side, deterministic disposition, and timestamp. Prior thesis text is deliberately excluded so earlier model prose cannot become later instructions. Financial credentials, provider account IDs, connection IDs, cost basis, unrelated positions, preview/order IDs, and raw provider payloads never enter the model request. The internal request uses the exact AI connection/model frozen into the mandate, a one-way safety identifier, no tools, a bounded output, and a strict schema. OpenAI and Gemini disable response storage per request; Anthropic uses its Messages JSON-output contract and the account's applicable provider retention controls. Gemini additionally requests no thinking summaries and explicitly disables tool choice.

The model may return `ABSTAIN`, or one `PROPOSE` decision containing an allowlisted symbol, BUY/SELL, and a positive notional no greater than the mandate ceiling. It is instructed to use the bounded recent-decision context to avoid repetitive reasoning, but that context is advisory evidence rather than authority. That memory preserves the exact non-live disposition: Shadow `WOULD_HAVE_SUBMITTED`, Paper `SIMULATED_FILLED` or `SIMULATED_REJECTED`, or a deterministic control hold. Python and Go accept the same closed disposition vocabulary and reject unknown values. Python validates the schema and envelope; Go revalidates price, quantity, mandate/model metadata, and current SELL availability. Abstention appends an immutable `AI / ABSTAIN` Decision Journal entry with no risk or execution record. A proposal enters the same provider-independent deterministic risk engine used by other financial actions. That gate also enforces a platform-owned one-hour cooldown on an identical autonomous SHADOW symbol and side, using only prior immutable `WOULD_HAVE_SUBMITTED` records from the same strategy instance; missing or invalid recent-action evidence fails closed. An allowed Shadow result reaches only the Shadow adapter and is persisted as `WOULD_HAVE_SUBMITTED`; a cooldown or other denial is persisted as `RISK_DENIED`. Paper dispositions remain isolated simulation evidence. Neither mode changes broker holdings, creates a live order, reserves live capital, or calls an order endpoint. Arbion's hourly decision budget is isolated per non-live strategy instance so one account cannot consume another account's evaluation capacity. The internal instance scope is never sent to the model. Exhausting that bounded window records a safe scheduled skip (`AI_DECISION_BUDGET_EXHAUSTED`) rather than a scheduler failure; an upstream model-provider throttle remains distinct as `AI_PROVIDER_RATE_LIMITED`. Both stop before any financial action, and provider error text is never returned.

The owner-scoped mandate journal joins each proposal to its existing deterministic risk and non-live execution evidence. New decisions freeze the AI provider, exact model, route profile, and the sanitized input evidence actually supplied to the model: financial provider name, available cash, buying power, allowlisted position quantity/availability/observed value, and the bounded market facts with their individual price and history provenance. For Coinbase that includes exact derived 1-hour and 6-hour changes plus the contiguous-candle coverage that made each window eligible. Credentials, provider account IDs, unrelated holdings, prompts, raw provider responses, and private chain-of-thought remain excluded. The UI may therefore show both the immutable model route, what the model considered, and the hypothetical side, exact quantity, reference price, notional, risk decision, and `WOULD_HAVE_SUBMITTED` status without inventing a fill, return, or performance claim. This evidence remains an immutable SHADOW record and is never a broker order.

Eligible successful AI cycles also create at most one immutable 1-hour and one immutable 24-hour outcome mark for each prior `WOULD_HAVE_SUBMITTED` record once the horizon is due. A hypothetical BUY is conservatively marked against the fresh bid; a hypothetical SELL is marked against the fresh ask, with an explicit mark/last fallback if the closing side is unavailable. Exact decimal math records only the directional USD and percentage movement from the stored reference price, along with elapsed time, feed, quality, pricing basis, and market timestamp. The mark excludes fees and slippage and is not a fill, realized return, account P&L, provider performance record, or execution authorization. Missing or stale market evidence fails closed before a new AI decision.

The owner-scoped SHADOW scorecard groups those immutable marks by 1-hour and 24-hour horizon. It reports exact sample, favorable, unfavorable, and flat counts plus observed favorable rate, average and discrete-median directional movement, best/worst observed marks, average marked USD value, and the sum of independent marked USD values. The behavior projection also keeps each proposed symbol's sample count, favorable/unfavorable/flat counts, favorable rate, average directional movement, and average marked USD value separate at each horizon; it never attributes an abstention to a symbol. Horizons and symbols never mix and missing marks are never estimated. Dollar summaries scale each stored movement by that proposal's hypothetical quantity; they do not reconstruct an account or portfolio, net overlapping proposals, add fees or slippage, assert a fill, or represent realized profit and loss. A separate evidence-collection gate stays `COLLECTING_EVIDENCE` until both horizons contain at least 20 marks, the observed marks span at least seven days, and the latest schedule state is `SUCCEEDED` with zero consecutive failures. Passing those transparent requirements changes only the gate to `EVIDENCE_REVIEWABLE`; it does not evaluate profitability, authorize promotion, create a live mode, or add broker-write capability. The per-horizon interpretation likewise remains `INSUFFICIENT_SAMPLE` until its 20-mark floor and then becomes `OBSERVATIONAL`, a descriptive label only—not prediction accuracy, statistical significance, realized performance, or authority to trade.

Once that exact gate is reviewable, the owner may append an MFA-backed `NON_LIVE_EVIDENCE_ONLY` acknowledgment from the automation review page. Arbion hashes a canonical, versioned snapshot containing the pinned instance and mandate version plus the complete aggregate scorecard, behavior, and gate evidence. The database stores only that 32-byte fingerprint and bounded control facts; it rejects updates and deletes. New evidence creates a different fingerprint, so an older acknowledgment remains visible as history but no longer marks the current snapshot reviewed. The acknowledgment cannot change a mandate, instance, schedule, reconciliation, capital policy, order intent, or execution boundary, and it never calls a financial or AI provider.

The same owner page reads those acknowledgments through a bounded reverse-chronological ledger with a stable opaque cursor. Every item remains immutable and is labeled current only by comparison with the separately computed current scorecard fingerprint; prior fingerprints remain explicit history. An unavailable ledger stays unavailable rather than being treated as no reviews. This read path contains no model invocation, provider call, broker operation, mandate or schedule mutation, order-intent creation, promotion control, or live execution authority.

The mandate review surface also provides a provider-neutral Decision Replay Lab over this existing immutable evidence. Its per-strategy history is owner-scoped and reverse-chronologically paginated with a stable opaque cursor; each bounded page carries only the 1-hour/24-hour marks whose execution records occur on that page. The browser appends older pages on explicit request and deduplicates immutable identifiers, so the replay may inspect every loaded record while the compact journal summary stays focused on the five newest. It filters proposals, abstentions, and deterministic control holds, then reconstructs the recorded model route, sanitized input snapshot, risk disposition, SHADOW adapter result, and exact outcome marks without rerunning the model. Missing inputs or outcomes remain explicitly unavailable or pending, and a failed history projection is not represented as an empty journal. Replay is read-only: it cannot revise history, request a fresh decision, preview an order, contact a financial provider, approve an action, or submit, replace, or cancel an order.

A separate Strategy Simulation Workbench compares any two owner-scoped immutable mandate versions and layers a browser-local what-if over the current proposal ceiling, maximum decisions per day, capital-bucket capacity, and allowlisted symbol. When an exact prior market observation exists in the Decision Journal, the workbench may calculate units at that recorded price and identify the observation's feed, quality, and timestamp; it never requests or substitutes a live quote. Its daily-notional and capacity-pressure calculations are configuration arithmetic only, not a backtest, forecast, approval, risk result, expected volume, or permission to trade. A scenario remains local unless the owner opens the explicit diff, confirms both bounded limit changes, and creates a new immutable DRAFT through the dedicated concurrency-checked endpoint. That endpoint preserves account, capital bucket, AI route, objective, universe, schedule, autonomy, SHADOW mode, and broker permissions; it cannot mark the version READY, initialize an instance, run a model, pass risk, or create an order. An already active non-live instance remains pinned to its immutable READY version and continues using that version's exact account, bucket, parameters, autonomy, schedule, and notification settings while a newer DRAFT is reviewed. The mutable mandate must still be READY or DRAFT, and explicit PAUSED, DISABLED, or ARCHIVED status, entitlement loss, an inactive account or bucket, an invalid pinned snapshot, or a non-Shadow mode remains fail-closed. The existing instance must be finished before a separately reviewed replacement version can be initialized.

## Connection lifecycle

Entitled users can create, rename, replace credentials for, verify, enable, disable, list, and delete their own AI connections. New connections and replacements for non-active connections enter `pending`; encryption success is not verification. Replacements for active connections instead use the staged verification and atomic promotion flow below. The documented lifecycle vocabulary is `pending`, `active`, `error`, `expired`, `revoked`, and `disabled`. Verification moves a pending connection to active or error after contacting only its registered provider adapter; failed verification preserves the stored credential so it can be replaced.

Disabling preserves the connection and encrypted credential but makes it ineligible for future Neural Engine use. Enabling returns it to pending eligibility and does not claim validity. Disablement fails before Vault or status mutation while a READY/PAUSED mandate or ACTIVE/PAUSED runtime depends on the connection; the runtime check follows its pinned immutable mandate version even if a newer DRAFT selects another connection. An active connection rotates without downtime: Arbion stores the replacement only as an encrypted token-bound candidate, continues serving the current verified key, verifies the candidate through the registered provider, and atomically promotes only the same status/generation/token candidate. A failed, stale, interrupted, or overlapping attempt cannot replace or disable the current key. Non-active replacements enter `pending` and require the normal explicit verification. Deletion first refuses connections referenced by a Neural preference, current mandate, or immutable mandate version, then removes the vault secret and connection. Ownership-filtered queries return the same not-found result for absent and foreign IDs.
