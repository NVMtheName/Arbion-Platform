...
The mandate review surface also provides a provider-neutral Decision Replay Lab over this existing immutable evidence. It filters proposals, abstentions, and deterministic control holds, then reconstructs the recorded model route, sanitized input snapshot, risk disposition, SHADOW adapter result, and exact 1-hour/24-hour marks without rerunning the model. Missing inputs or outcomes remain explicitly unavailable or pending. Replay is read-only: it cannot revise history, request a fresh decision, preview an order, contact a financial provider, approve an action, or submit, replace, or cancel an order.
