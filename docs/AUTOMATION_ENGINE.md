...
Future superadmin operations may show automation, worker, execution, provider, and reconciliation health. They must not reveal plaintext user credentials. Automation-, account-, and owner-wide user-level emergency controls are implemented as audited owner actions. The account control applies only to the selected owner/account boundary, while the owner-wide control applies only to accounts and automations belonging to the authenticated owner. Neither can stop or release another user's resources. The global control remains a future administrative action; administrative authority does not bypass user boundaries or safety policy.
