...
Arbion retains the supported Docker Compose/Caddy single-host deployment for simpler operations. The current owner-operated production topology is Lightsail; use the host-safe `scripts/deploy-lightsail-release.sh` helper and [the production deployment runbook](docs/DEPLOYMENT.md) for releases. The intended scalable production foundation is AWS ECS/Fargate with a public ALB and private application/data tiers, defined in Terraform without applying resources. Its separate manual workflow must not be used until that stack is deliberately provisioned and reviewed. Neither topology enables live trading.
