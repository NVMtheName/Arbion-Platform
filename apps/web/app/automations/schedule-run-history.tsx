    ...
    <section className="schedule-run-history" aria-label="Scheduler run ledger">
      ...
      {runs.length === 0 ? (
        ...
        <ol className="schedule-run-list">
          {runs.map((run) => {
            const recovery = run.error_code
              ? scheduleFailureGuidance(run.error_code, financialProvider)
              : undefined;
            return (
              <li className={`is-${run.status.toLowerCase()}`} key={run.id}>
                ...
                {recovery && (
                  <div
                    className={`schedule-run-guidance is-${recovery.action.toLowerCase().replace("_", "-")}`}
                  >
                    <strong>{recovery.actionLabel}</strong>
