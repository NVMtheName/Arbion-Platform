...
describe("StrategyFleet", () => {
  ...
  it("surfaces exact Paper guardrail attribution in the command deck", () => {
        ...
        items={[
          {
            ...
            paperGuardrailEvidence: {
              ...
              denial_eligibility: {
                ...
                first_denial_at: "2026-08-31T14:00:00Z",
                latest_denial_at: "2026-08-31T14:00:00Z",
                denials: [
                  {
                    decision_journal_entry_id: "decision-denied",
                    proposed_action_id: "action-denied",
    ...
    expect(
      within(guardrails).getByText(/denied proposal chronology/i),
    ).toBeInTheDocument();
    expect(
      within(guardrails).getByText(
        /no later comparable proposal in the saved window/i,
      ),
    ).toBeInTheDocument();
    expect(
      within(guardrails).getByRole("link", {
        name: /review exact denial evidence/i,
      }),
    ).toHaveAttribute("href", "/automations/paper-mandate#runtime-evidence");
  });
});
